...
namespace NHyphenator
{
    public class Hyphenator
    {
        ...
        private static Regex createMaskRegex = new Regex(@"\w");
        ...
        private void LoadPatterns(HyphenatePatternsLanguage language)
        {
            ...
            switch (language)
            {
                case HyphenatePatternsLanguage.EnglishBritish:
                    {
                        CreatePatterns(Patterns.hyph_en_gb_pat, Patterns.hyph_en_gb_hyp);
                        break;
                    }
                ...
                case HyphenatePatternsLanguage.Russian:
                    {
                        CreatePatterns(Patterns.hyph_de_ch_pat, Patterns.hyph_de_ch_hyp);
                        break;
                    }
                case HyphenatePatternsLanguage.GermanSwitzerland:
                    {
                        CreatePatterns(Patterns.hyph_de_ch_pat, Patterns.hyph_de_ch_hyp);
                        break;
                    }
        ...
        private string FindLastWord(string phrase)
        {
            ...
            for (int i = phrase.Length - 1; i >= 0; i--)
            {
                ...
                else if (currentWord.Length > 0 && anyIsLetter(currentWord.ToString()))
                    return reverse(currentWord.ToString());
                else
                    currentWord.Append(phrase[i]);
            }
        ...
        private static bool anyIsLetter(string str)
        {
        ...
        private Pattern CreatePattern(string pattern)
        {
            ...
            foreach (char c in pattern)
            {
                if (Char.IsDigit(c))
                {
                    ...
                    levels.Add(Int32.Parse(c.ToString()));
                    waitDigit = false;
                }
                else
                {
